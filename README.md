...
| Agent | 要求 |
|------|------|
| 开发 Agent（本机） | 技能 `code-change-review-cycle`，所有写代码任务自动执行 |
| 协作 Agent（远程设备） | 按此文档执行，写代码必须走 5步法，审用 OpenCode |
| 安全 Agent | 涉及代码改动时按此流程，安全审查结果也按 severity 分级 |
| 通用助手 Agent | 按此文档执行 |
...
- 审核编排细节: 内部技能 references/opencode-review-orchestration.md
